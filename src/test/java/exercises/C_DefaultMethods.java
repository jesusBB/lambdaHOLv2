...
public class C_DefaultMethods {
    ...
    public void c04_appendToMapValues() {
        ...
        map.entrySet()
                .stream()
                .map(e -> e.getValue().append(e.getKey()))
                .collect(Collectors.toList());

        assertEquals(3, map.size());
        assertTrue(map.values()
                .stream()
                .allMatch(x -> x instanceof StringBuilder));
    ...
    public void c06_mapOfListOfStringsByLength() {
        ...
        list.forEach(
                s -> result.computeIfAbsent(s.length(), k -> new ArrayList<>())
                        .add(s));
    ...
    public void c07_mapOfStringByInitialCharacter() {
        ...
        // TODO write code to populate result
        list.forEach(word -> result.merge(word.charAt(0), word,
                (s1, s2) -> s1 + ":" + s2));
    ...
    public void c11_computeWithMissingEntries() {
        ...
        keys.forEach(key -> map.compute(key,
                (k, v) -> v == null ? k : v.toUpperCase()));
        assertEquals(Map.of("a", "ALFA",
                "b", "BRAVO",
                "c", "CHARLIE",
                "d", "DELTA",
    ...
    public void c12_computeAndRemoveSomeEntries() {
        ...
        // TODO write code transform the map
        keys.forEach(key -> map.compute(key,
                (k, v) -> !v.isEmpty() ? v.toUpperCase() : null));
