...
public class E_IntermediateStreams {
    ...
    public void e3_longLowerCaseSortedWords() throws IOException {
        ...
        output = reader.lines()
                .flatMap(line -> SPLIT_PATTERN.splitAsStream(line))
                .filter(word -> ((String) word).length() >= 8)
   ...
   // @Ignore
    ...
    public void e5_sortedLowerCaseDistinctByLengthThenAlphabetically()
        ...
        //result = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).map(String::toLowerCase).distinct().sorted(Comparator.comparing(String::length)).collect(Collectors.toList());
        result = reader.lines()
                .flatMap(line -> SPLIT_PATTERN.splitAsStream(line))
                .map(String::toLowerCase)
                .distinct()
        ...
        assertEquals(
                List.of(
                        ...
                        "his", "now", "own", "the", "thy", "too", "bear",
                        "else",
                        "eyes", "from", "fuel", "heir", "lies", "only",
                        "pity", "rose", "self", "that", "thee", "this", "thou",
                        "time", "with", "churl", "cruel", "flame", "fresh",
                        "gaudy",
                        "grave", "might", "never", "riper", "sweet", "thine",
                        "waste", "where", "world", "bright", "desire", "famine",
                        "herald", "mak'st", "making", "memory", "should",
                        "spring",
                        "tender", "within", "buriest", "content", "decease",
                        "fairest", "feed'st", "glutton", "light's", "thereby",
                        "world's", "beauty's",
                        "increase", "ornament", "abundance", "creatures",
                        "contracted", "niggarding",
                        "substantial"),
                result);
    }
    // Hint 1:
    ...
    public void e6_bigFactorial() {
        ...
        result = LongStream.rangeClosed(1, 21).mapToObj(BigInteger::valueOf).reduce(BigInteger.ONE, BigInteger::multiply);
    ...
    public void e7_getLastWord() throws IOException {
        ...
        result = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).reduce((a, b) -> b).orElse("");
    ...
    public void e8_selectTheSuperClassesOfArrayList() {
        ...
        //List<Class> result = Stream.iterate(origin, Class::getSuperclass).takeWhile(Objects::nonNull).collect(Collectors.toList());
      //  Stream.iterate(origin, Class::getSuperclass).forEach(System.out::println);
        assertEquals(
                List.of(ArrayList.class, AbstractList.class,
