...
public class E_IntermediateStreams {
    ...
    public void e1_stringsToCharacters() {
        final List<String> input = List.of("alfa", "bravo", "charlie");

        final List<Character> result = input.stream().flatMap(s -> s.chars().mapToObj(i -> (char) i)).collect(Collectors.toList()); // TODO
    ...
    public void e2_listOfAllWords() throws IOException {
        final List<String> output = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).collect(Collectors.toList()); // TODO
    ...
    public void e3_longLowerCaseSortedWords() throws IOException {
        final List<String> output = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).filter(word -> word.length() >= 8)
    ...
    public void e4_longLowerCaseReverseSortedWords() throws IOException {
        final List<String> result = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).filter(word -> word.length() >= 8)
                .map(String::toLowerCase).sorted(Comparator.reverseOrder()).collect(Collectors.toList()); // TODO
    ...
    public void e5_sortedLowerCaseDistinctByLengthThenAlphabetically() throws IOException {
        final List<String> result = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).map(String::toLowerCase).distinct()
    ...
    public void e6_bigFactorial() {
        final BigInteger result = LongStream.rangeClosed(1, 21)
    ...
    public void e7_getLastWord() throws IOException {
        final String result = reader.lines().flatMap(line -> SPLIT_PATTERN.splitAsStream(line)).reduce((a, b) -> b).orElse(""); // TODO
    ...
    public void e8_selectTheSuperClassesOfArrayList() {
        final Class<?> origin = ArrayList.class;

        final List<Class<?>> result = Stream.<Class<?>> iterate(origin, Class::getSuperclass).takeWhile(Objects::nonNull).collect(Collectors.toList()); // TODO
    ...
    public void e9_countTheElementsAfterAPredicate() {
        ...
        final Random rand = new Random(314L);
        final Stream<String> stream = Stream.iterate(
                "",
                (String s) -> {
